...
class ShuffleNetV2_Plus(nn.Module):
    def __init__(self, input_size=224, n_class=1000, architecture=None, model_size='Small'):
    ...
    def _initialize_weights(self):
        for name, m in self.named_modules():
            if isinstance(m, nn.Conv2d):
                ...
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
def ShuffleNetV2p(n_class):
    architecture = [0, 0, 3, 1, 1, 1, 0, 0, 2, 0, 2, 1, 1, 0, 2, 0, 2, 1, 3, 2]
    return ShuffleNetV2_Plus(n_class=n_class, architecture=architecture)
